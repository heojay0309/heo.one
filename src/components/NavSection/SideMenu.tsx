...
const SideMenu = ({ activeSection }: SideMenuProps) => {
  ...
  return (
    <div
      className={`fixed top-[490px] hidden h-full tablet:right-[4px] tablet:block tablet:gap-[64px] laptop:right-[64px]`}
    >
      <div
        className={`${path === "/resume" && "hidden"} transition-opacity duration-500 ${activeSection === "intro" ? "opacity-0" : "opacity-100"} flex flex-col items-center gap-[16px] rounded-lg bg-white bg-opacity-10 hover:bg-opacity-20 tablet:gap-[16px] tablet:px-[8px] tablet:py-[16px]`}
      >
        <Link
          href={"#intro"}
